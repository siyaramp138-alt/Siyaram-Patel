...
import { PhoneCall, Users, CalendarCheck, Megaphone, Settings, BarChart2, Cpu, Sparkles, Volume2 } from 'lucide-react';
    ...
    <header className="bg-[#08080a]/90 backdrop-blur-md border-b border-slate-800/80 text-white sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Call Status & Quick Action Button */}
          <div className="flex items-center space-x-3">
            <div className="hidden sm:flex bg-slate-900 border border-slate-800 px-3.5 py-1.5 rounded-full items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
              <span className="text-[11px] font-medium uppercase tracking-widest text-slate-300">Systems Active</span>
            </div>
            ...
            {isCallActive ? (
              ...
              <button
                onClick={onQuickCall}
                className="bg-white text-black hover:bg-slate-200 px-5 py-2 rounded-full text-xs font-bold transition-all shadow-md flex items-center space-x-1.5"
              >
                <Sparkles className="w-3.5 h-3.5 text-indigo-600" />
                <span>TEST CALL SIYA</span>
              </button>
            )}
          </div>
        </div>
